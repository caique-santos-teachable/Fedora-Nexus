...
URL, route `module:`, file path and Ruby namespace must stay in sync. The sample below shows the convention — apply the same pattern for any new resource.

```
app/controllers/public_api/admin_api/v2/
├── application_controller.rb              # base controller (response helpers, auth)
└── products/
    ├── <resource>_controller.rb           # top-level resource
    └── <resource>/
        └── <nested_resource>_controller.rb  # nested resource

app/services/public_api/v2/
├── base_handler.rb                        # BaseHandler (CRUD helpers, monads)
├── <resource>_handler.rb                  # simple handler (one model)
├── <domain>/
│   └── <resource>_handler.rb             # composite/nested handler
└── queries/
    └── <domain>/<resource>/
        └── query_processor.rb            # filters + pagination (non-trivial queries only)

app/serializers/public_api/v2/
├── base_product_serializer.rb            # shared product attributes
...
## 6.5 Response JSON Shapes

### Success — Collection (index with pagination)
```json
{
  "data": [
    { "id": 1, "name": "Course Name", "description": "...", "is_published": true, "created_at": "...", "updated_at": "..." }
  ],
  "meta": { "page": 1, "per_page": 25, "total_pages": 4, "total_count": 100 }
}
```
...
```json
{
  "data": { "id": 1, "name": "Course Name", "description": "...", "is_published": true, "created_at": "...", "updated_at": "..." }
}
```
...
```json
{
  "error": { "code": "validation_failed", "message": "The request could not be processed", "details": { "name": ["can't be blank"] } }
}
```
...
> Accumulated guardrails (anti-patterns, N+1 rules, multi-tenancy, rswag discipline) live in **`public-api-v2-guardrails.instructions.md`**. Load it alongside this file for any implementation task.
