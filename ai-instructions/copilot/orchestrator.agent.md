...
name: Orchestrator
tools: [read, search, agent, todo, memory]
agents: [Engineer, QA, Improvement]
...
2. Search relevant files to understand context (models, controllers, serializers, tests).
3. Write a concise implementation brief: what needs to be done, where, and acceptance criteria.
4. Create a todo list with all phases tracked.
...
4. For each `new_skills` entry in the improvement plan:
   - Create the skill file at `/Users/caiquesantos/Library/Application Support/Code/User/prompts/skills/<name>/SKILL.md` (global Copilot folder — never inside the repository).
