...
2) Skills, agents, rules e improvements — escrita obrigatória na pasta global do Copilot
- **Anti-pattern**: criar arquivos de skill (`SKILL.md`), agent (`.agent.md`), instruction (`.instructions.md`) ou qualquer saída do Improvement agent dentro do repositório (ex.: `.github/skills/<name>/SKILL.md`) — esses arquivos ficam acoplados ao repo, não são reutilizáveis em outros projetos e podem ser commitados por acidente.
- **Padrão recomendado**: toda skill, agent, rule ou improvement **deve ser escrita exclusivamente na pasta global do Copilot** (`/Users/caiquesantos/Library/Application Support/Code/User/prompts/`). Nunca usar caminhos relativos ao workspace para esses artefatos.
- **Regra de ouro**: se o arquivo pertence ao sistema de agentes (não ao código do produto), ele vai para a pasta global, não para o repo.
- **Checklist antes de criar/modificar artefato do sistema**:
  1. O arquivo é uma skill, agent, instruction ou improvement? → pasta global obrigatória.
  2. O path começa com `.github/` ou qualquer caminho relativo ao repo? → erro estrutural, corrigir.
- **Mapeamento de paths corretos**:
  | Tipo | Path correto |
  |------|-------------|
...
- **Exemplo**:
  ```
  # ❌ skill no repo — acoplada ao projeto, não reutilizável
  .github/skills/dead-code/SKILL.md

  # ✅ skill na pasta global — disponível em qualquer workspace
  /Users/caiquesantos/Library/Application Support/Code/User/prompts/skills/dead-code/SKILL.md
  ```
