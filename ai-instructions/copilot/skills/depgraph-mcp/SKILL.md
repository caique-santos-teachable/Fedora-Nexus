---
name: depgraph-mcp
description: "Use when: querying or indexing a codebase with the depgraph MCP server. Covers all tools (index_repo, search, get_dependencies, get_dependents, blast_radius, query_graph, get_graph, list_repos, delete_repo), the Kuzu graph schema, and FTS search examples."
---

# Skill: depgraph MCP

## Context
depgraph is an MCP server that builds and queries a dependency graph for source code repositories. It supports Python, TypeScript, JavaScript, and Ruby. Agents use it to understand impact before editing, search symbols by name/content, detect circular dependencies, and navigate large codebases.

The server runs as a persistent HTTP/SSE process (`depgraph-server` Docker container) on port 7832, or as a one-shot stdio process (`depgraph-stdio`).
...
|---|---|---|---|---|
| `root_path` | yes | string | — | HOST-side absolute path (e.g. `/Users/caiquesantos/code/fedora`) |
