...
depgraph search /path/to/repo "payment validation error" --limit 20

# 2. Find callers via Cypher
depgraph query /path/to/repo "MATCH (caller)-[r:CodeRelation {type: 'CALLS'}]->(f:Function {name: 'validate_payment'}) RETURN caller.name, caller.file_path"

# 3. Trace import chain
depgraph deps /path/to/repo src/payments/validators.py --depth 3

# 4. Map recent change impact
depgraph blast-radius /path/to/repo src/payments/validators.py --max-depth 3
```
