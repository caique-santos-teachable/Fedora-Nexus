...
# Safe Refactoring with depgraph
...
depgraph search /path/to/repo "validate_user"

# 3. Precise caller list via Cypher
depgraph query /path/to/repo "MATCH (caller)-[r:CodeRelation {type: 'CALLS'}]->(f:Function {name: 'validate_user'}) RETURN caller.name, caller.file_path, caller.start_line ORDER BY caller.file_path"

# 4. All importers to update
depgraph dependents /path/to/repo src/auth/validators.py --depth 3

# 5. Verify after refactor (re-index with force)
depgraph index /path/to/repo --force
depgraph blast-radius /path/to/repo src/auth/validators.py
```
