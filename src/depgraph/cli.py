"""CLI interface for depgraph — thin HTTP client for the depgraph container server.

Mode selection (in priority order):
  1. DEPGRAPH_SERVER_URL env var is set  → HTTP client mode (talks to container)
...
def _server_url() -> str | None:
    ...
    from_env = os.environ.get("DEPGRAPH_SERVER_URL", "").rstrip("/")
    if from_env:
        return from_env
...
def _http_call(server_url: str, tool: str, args: dict) -> dict:
    ...
    try:
        ...
        return {"error": f"Cannot reach depgraph server at {server_url}: {exc.reason}", "code": "SERVER_UNREACHABLE"}
...
def _local_call(tool: str, args: dict) -> dict:
    ...
    from depgraph.mcp.server import (  # noqa: PLC2701
        _tool_blast_radius,
        _tool_delete_repo,
...
@click.option(
    "--server",
    envvar="DEPGRAPH_SERVER_URL",
    default=None,
    metavar="URL",
    help="depgraph server URL (e.g. http://localhost:7832). Auto-detected if omitted. "
         "Set DEPGRAPH_SERVER_URL to force a specific server.",
)
@click.pass_context
def main(ctx: click.Context, pretty: bool, server: str | None) -> None:
    """depgraph — code dependency graph CLI for AI agents.

    Every command outputs JSON to stdout.
    Exit code 0 = success, 1 = error.

    By default the CLI talks to the depgraph container server running on
    ...
    \b
    Force server URL:
        DEPGRAPH_SERVER_URL=http://my-host:7832 depgraph list
    """
...
def cmd_query(ctx: click.Context, root_path: str, cypher: str) -> None:
    ...
    Example:
        ...
        depgraph query /path/to/repo "MATCH (c:Class {name:'UserService'})-[r:CodeRelation]->(m:Method) RETURN m.name"
    """
...
def _compose_file() -> Path:
    """Return the absolute path to the docker-compose.yml bundled with depgraph."""
    ...
    raise FileNotFoundError(
        ...
        "Make sure you installed depgraph from the repository, not a wheel."
    )


def _docker_compose(*args: str) -> int:
    """Run `docker compose` with the depgraph compose file, streaming output."""
    try:
...
def cmd_server_start(build: bool) -> None:
    """Start the depgraph container server (docker compose up).
    ...
    if rc == 0:
        click.echo("depgraph server started. Run 'depgraph list' to verify.")
    sys.exit(rc)
...
def cmd_server_stop() -> None:
    """Stop the depgraph container server (docker compose stop)."""
...
def cmd_server_remove(volumes: bool) -> None:
    """Remove the depgraph container (docker compose down).
...
def cmd_server_status(ctx: click.Context) -> None:
    """Show whether the depgraph server is reachable and return its /health response."""
