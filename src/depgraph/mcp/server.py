"""MCP server exposing depgraph tools for AI agents."""
...
from depgraph.graph.blast_radius import blast_radius as _blast_radius
...
def _run_http() -> None:
    ...
    port = int(_os.environ.get("DEPGRAPH_HTTP_PORT", "7832"))
    ...
    async def handle_call(request: Request) -> JSONResponse:
        """JSON-RPC style endpoint used by the depgraph CLI.
...
def main() -> None:
    import sys
    logger.info("depgraph MCP server starting — PID=%d", _os.getpid())
    try:
