...
from depgraph.store import embedding_store as _emb

logger = logging.getLogger(__name__)

_DEFAULT_DB_PATH = "/data/depgraph.db"  # used inside Docker; overridden by DEPGRAPH_DB_PATH

def _default_db_path() -> str:
    """Return the platform-appropriate default database path.

    Priority:
    1. DEPGRAPH_DB_PATH env var (explicit override, used by Docker/CI)
    2. ~/.local/share/depgraph/depgraph.db (local CLI default)

    The /data/depgraph.db constant is kept for backward compat with Docker mounts.
    """
    from_env = os.environ.get("DEPGRAPH_DB_PATH", "")
    if from_env:
        return from_env
    local_dir = Path.home() / ".local" / "share" / "depgraph"
    local_dir.mkdir(parents=True, exist_ok=True)
    return str(local_dir / "depgraph.db")
    ...
    def init_schema(self) -> None:
        ...
        try:
            ...
            raise SchemaVersionError(
                "Old schema detected. Delete the DB and restart: rm -rf /data/depgraph.db"
            )
