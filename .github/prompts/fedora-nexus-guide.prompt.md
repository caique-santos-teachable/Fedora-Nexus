---
mode: 'agent'
description: 'Reference guide for all fedora-nexus MCP tools, graph schema, and workflow tips. Use when asked about available fedora-nexus tools or how to query the graph.'
---
...
fedora-nexus index /path/to/repo --force          # force re-index

# Search symbols
fedora-nexus search /path/to/repo "UserService"
fedora-nexus search /path/to/repo "authenticate" --limit 30
...
# Blast radius
fedora-nexus blast-radius /path/to/repo src/auth.py src/user.py --max-depth 3

# Cypher query (native Kuzu)
fedora-nexus query /path/to/repo "MATCH (f:Function {name: 'authenticate'}) RETURN f.file_path, f.start_line"

# Full graph export
fedora-nexus graph /path/to/repo --json
```
