...
```bash
# 1. Search for the suspect symbol or error keyword
fedora-nexus search /path/to/repo "payment validation error" --limit 20

# 2. Find callers via Cypher
fedora-nexus query /path/to/repo "MATCH (caller)-[r:CodeRelation {type: 'CALLS'}]->(f:Function {name: 'validate_payment'}) RETURN caller.name, caller.file_path"

# 3. Trace import chain
fedora-nexus deps /path/to/repo src/payments/validators.py --depth 3

# 4. Map recent change impact
fedora-nexus blast-radius /path/to/repo src/payments/validators.py --max-depth 3
```
