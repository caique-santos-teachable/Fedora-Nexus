...
# Exploring Codebases with fedora-nexus
...
fedora-nexus search /path/to/repo "payment processing" --limit 20

# 4. Cypher deep dive — callers of a function
fedora-nexus query /path/to/repo "MATCH (caller)-[r:CodeRelation {type: 'CALLS'}]->(f:Function {name: 'processPayment'}) RETURN caller.name, caller.file_path"

# 5. File-level import chain
fedora-nexus deps /path/to/repo src/payments/processor.py --depth 2
```
