...
depgraph blast-radius /path/to/repo src/auth/validators.py src/auth/login.py --max-depth 3
...
depgraph query /path/to/repo "MATCH (caller)-[r:CodeRelation {type: 'CALLS'}]->(f:Function {name: 'validate_token'}) RETURN caller.name, caller.file_path"

# 5. Find any missed references
depgraph search /path/to/repo "validate_token"
```
