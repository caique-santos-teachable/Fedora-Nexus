...
```bash
# 1. Get changed files (standard git)
git diff --name-only origin/main...HEAD

# 2. Full blast radius for all changed files
fedora-nexus blast-radius /path/to/repo src/auth/validators.py src/auth/login.py --max-depth 3

# 3. Direct importers of a key changed file
fedora-nexus dependents /path/to/repo src/auth/validators.py --depth 2

# 4. Callers of a changed symbol
fedora-nexus query /path/to/repo "MATCH (caller)-[r:CodeRelation {type: 'CALLS'}]->(f:Function {name: 'validate_token'}) RETURN caller.name, caller.file_path"

# 5. Find any missed references
fedora-nexus search /path/to/repo "validate_token"
```
